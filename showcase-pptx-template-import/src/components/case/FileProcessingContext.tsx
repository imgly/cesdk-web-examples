...
const STATUS_MESSAGES = {
  ...
  processing: 'Processing: Transforming PPTX to Scene',
  done: '',
...
const FileProcessingContextProvider = ({
  ...
  const processFile = useCallback(async (file: ExampleFile) => {
    ...
    try {
      ...
      // Export first page as preview
      const pages = engine.scene.getPages();
      if (pages.length > 0) {
        imageBlob = await engine.block.export(pages[0], {
          mimeType: 'image/png',
          targetHeight: 1000,
          targetWidth: 1000
        });
    ...
    // Validate that we have valid results before setting
    if (!imageBlob || !sceneArchive) {
      console.error('Failed to process PPTX: Missing image or scene archive');
      resetState();
      return;
    }

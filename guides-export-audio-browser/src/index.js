...
import { exportAudioFromBlock, exportAudioWithUI, downloadAudioBlob } from './exportAudio.js';
import { transcribeAudio } from './transcriptionService.js';
import { importCaptionsToScene } from './importCaptions.js';
import { runWorkflowWithUI, loadSampleVideo } from './completeWorkflow.js';
...
function setupEventHandlers() {
  ...
  document.getElementById('loadSampleVideo').addEventListener('click', async () => {
    const statusText = document.getElementById('statusText');
    try {
  ...
  document.getElementById('findAudioSources').addEventListener('click', () => {
    ...
    if (sources.length > 0) {
      document.getElementById('exportAudio').disabled = false;
      document.getElementById('statusText').textContent = `Found ${sources.length} audio sources`;
    } else {
      document.getElementById('statusText').textContent = 'No audio sources found';
    }
  });
  ...
  document.getElementById('exportAudio').addEventListener('click', async () => {
    if (!selectedSourceId) {
      document.getElementById('statusText').textContent = 'Please select an audio source first';
      return;
    }
    ...
    try {
      ...
      if (blockType === '//ly.img.ubq/fill/video') {
        ...
        if (parentBlock) {
          ...
          console.log('Exporting from parent graphic block:', exportBlockId, 'instead of fill:', selectedSourceId);
        }
      }

      currentAudioBlob = await exportAudioWithUI(cesdk, exportBlockId, progressBar);

      // Enable transcription button
      document.getElementById('transcribeAudio').disabled = false;

      // Option to download the audio
      const shouldDownload = confirm('Audio exported! Do you want to download it?');
      if (shouldDownload) {
        downloadAudioBlob(currentAudioBlob, 'cesdk-export.wav');
      }
    } catch (error) {
      document.getElementById('statusText').textContent = `Export failed: ${error.message}`;
  ...
  document.getElementById('transcribeAudio').addEventListener('click', async () => {
    if (!currentAudioBlob) {
      document.getElementById('statusText').textContent = 'Please export audio first';
      return;
    }
    ...
    try {
      ...
      const sourceName = selectedSourceId ? cesdk.engine.block.getName(selectedSourceId) : 'Unknown';
  ...
  document.getElementById('addCaptions').addEventListener('click', async () => {
    if (!currentTranscription) {
      document.getElementById('statusText').textContent = 'Please transcribe audio first';
      return;
    }
    ...
    try {
      const captionBlocks = await importCaptionsToScene(cesdk, currentTranscription.content, selectedSourceId);
  ...
  document.getElementById('getAudioTrackCount').addEventListener('click', async () => {
    if (!currentVideoFillId) {
      document.getElementById('statusText').textContent = 'Please load a video first';
      return;
    }

    const statusText = document.getElementById('statusText');
    try {
      const trackCount = cesdk.engine.block.getAudioTrackCountFromVideo(currentVideoFillId);
      statusText.textContent = `Video has ${trackCount} audio track${trackCount !== 1 ? 's' : ''}`;
  ...
  document.getElementById('getAudioTrackInfo').addEventListener('click', async () => {
    if (!currentVideoFillId) {
      document.getElementById('statusText').textContent = 'Please load a video first';
      return;
    }
    ...
    const statusText = document.getElementById('statusText');
    const trackInfoContainer = document.getElementById('audioTrackInfoContainer');
    const trackInfoElement = document.getElementById('audioTrackInfo');

    try {
      audioTrackInfo = cesdk.engine.block.getAudioInfoFromVideo(currentVideoFillId);
      statusText.textContent = `Retrieved information for ${audioTrackInfo.length} audio track${audioTrackInfo.length !== 1 ? 's' : ''}`;
  ...
  document.getElementById('createAudioFromVideo').addEventListener('click', async () => {
    if (!currentVideoFillId) {
      document.getElementById('statusText').textContent = 'Please load a video first';
      return;
    }
    ...
    const statusText = document.getElementById('statusText');
    try {
      statusText.textContent = 'Creating audio blocks from all video tracks...';
      ...
      const audioBlocks = cesdk.engine.block.createAudiosFromVideo(currentVideoFillId);
      ...
      statusText.textContent = `Created ${audioBlocks.length} audio block${audioBlocks.length !== 1 ? 's' : ''} from video tracks`;
  ...
  document.getElementById('runFullWorkflow').addEventListener('click', async () => {
    const statusText = document.getElementById('statusText');
    const progressBar = document.getElementById('progressBar');
...
function syncAudioSourceSelection() {
  ...
  if (audioSourceId) {
    ...
    sourceItems.forEach(item => {
      const itemBlockId = parseInt(item.dataset.blockId);
    ...
    if (!found) {
      console.log('Audio source not found in list. Available sources:', sourceItems.length);
    }
  }
}
...
function displayAudioSources(sources) {
  ...
  sources.forEach((source, index) => {
    ...
    item.addEventListener('click', () => {
      ...
      // Select in CE.SDK
      selectAudioSource(cesdk, source.id);

      // Enable export button
      document.getElementById('exportAudio').disabled = false;
      document.getElementById('statusText').textContent = `Selected: ${source.name}`;
    });

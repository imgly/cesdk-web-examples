...
Estilize-as como desejar.`,
  ...
  // Spanish (Track 3 or "Audio Track 3")
  spa: `1
00:00:00,160 --> 00:00:01,800
Bienvenido a CE.SDK.
...
2
00:00:02,760 --> 00:00:04,880
Exporta audio de cualquier bloque.
...
4
00:00:08,600 --> 00:00:10,160
Personalízalos como quieras.`
};

export async function transcribeAudio(audioBlob: Blob, options: any = {}) {
...
function mockTranscriptionService(audioBlob, sourceName) {
  ...
  if (sourceName) {
    if (sourceName.includes('Audio Track 1') || sourceName.includes('deu')) {
      ...
      sourceName.includes('Audio Track 2') ||
      sourceName.includes('por')
    ) {
      language = 'por';
      srtContent = MOCK_TRANSCRIPTIONS.por;
      console.log('→ Detected Portuguese (por) based on source name');
    } else if (
      sourceName.includes('Audio Track 3') ||
      sourceName.includes('spa')
    ) {
      language = 'spa';
...
export function validateSRTFormat(srtContent) {
  ...
  while (lineIndex < lines.length) {
    ...
    const sequenceNumber = parseInt(lines[lineIndex], 10);
    if (Number.isNaN(sequenceNumber)) {
      console.warn(
        `Invalid sequence number at line ${lineIndex}: ${lines[lineIndex]}`
      );
      lineIndex++;
      continue;
    }
    ...
    if (lineIndex >= lines.length || !timestampRegex.test(lines[lineIndex])) {
      console.warn(
        `Invalid timestamp at line ${lineIndex}: ${lines[lineIndex]}`
      );
      lineIndex++;
      continue;
    }

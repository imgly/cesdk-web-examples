...
export async function runCompleteAudioToCaptionWorkflow(cesdk, options = {}) {
  const workflow = {
    audioSources: [],
    selectedSource: null,
    audioBlob: null,
  ...
  try {
    ...
    if (options.sourceId) {
      ...
      const preferredSource = workflow.audioSources.find(source =>
    ...
    workflow.audioBlob = await exportAudioFromBlock(cesdk, workflow.selectedSource, {
    ...
    const sourceObject = workflow.audioSources.find(s => s.id === workflow.selectedSource);
    ...
    console.log(`Transcription complete: ${workflow.transcription.content.length} characters`);
    ...
    console.log(`Added ${workflow.captionBlocks.length} caption blocks to the scene`);
...
export async function runWorkflowWithUI(cesdk, uiElements) {
  ...
  if (selectedSourceElement) {
    options.sourceId = parseInt(selectedSourceElement.dataset.blockId);
  }
...
export async function loadSampleVideo(cesdk) {
  ...
  const sampleVideoUrl = 'https://img.ly/static/ubq_video_samples/multitrack_output.mp4';
  ...
  const totalDuration = cesdk.engine.block.getAVResourceTotalDuration(videoFill);
  ...
  console.log('Loaded sample video with audio (16:9 format, on background track, full duration, looping enabled)');

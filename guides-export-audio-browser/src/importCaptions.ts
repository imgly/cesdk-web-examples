...
export async function importCaptionsToScene(
  ...
  console.log(
    `Importing SRT content (length: ${srtContent.length} chars):`,
  ...
  try {
    ...
    let captionTrack: number | null = getCaptionTrack(cesdk);
    if (captionTrack) {
    ...
    console.log(
      'Created base64 data URL with mime type: text/srt, length:',
      captionURI.length
    );
    console.log(
      'Base64 encoded SRT (first 100 chars):',
      base64SRT.substring(0, 100)
    );

    // Verify decoding works correctly
    const decodedBytes = Uint8Array.from(atob(base64SRT), (c) =>
      c.charCodeAt(0)
    );
    ...
    const captionBlocks =
      await cesdk.engine.block.createCaptionsFromURI(captionURI);
    ...
    if (captionBlocks.length > 0) {
      try {
        const firstCaptionText = cesdk.engine.block.getString(
          captionBlocks[0],
          'text/text'
        );
        ...
        console.log(
          'Could not read first caption text:',
    ...
    captionBlocks.forEach((captionBlock: number) => {
    ...
    console.log(
      `Imported and added ${captionBlocks.length} caption blocks to the caption track`
    );
...
function muteAllAudioSourcesExcept(
  cesdk: CreativeEditorSDK,
  ...
  allBlocks.forEach((blockId: number) => {
    try {
      ...
      if (blockType === '//ly.img.ubq/audio') {
        ...
        console.log(
          `${shouldMute ? 'Muted' : 'Unmuted'} audio block:`,
          blockId
        );
      }
      ...
      if (blockType === '//ly.img.ubq/graphic') {
        try {
          ...
          if (fillType === '//ly.img.ubq/fill/video') {
            ...
            console.log(
              `${shouldMute ? 'Muted' : 'Unmuted'} video fill from graphic:`,
              fill
            );
          }
        } catch (e) {
...
async function applyComicPresetToCaptions(
  cesdk: CreativeEditorSDK,
  captionBlocks: number[]
) {
  ...
  try {
    ...
    const comicAsset = result.assets.find(
      (asset: { id: string }) => asset.id === comicPresetId
    );
    ...
    await cesdk.engine.asset.applyToBlock(
...
function getCaptionTrack(cesdk: CreativeEditorSDK): number | null {
  const page = cesdk.engine.scene.getCurrentPage();
  if (!page) return null;
...
function styleCaptionBlock(cesdk: CreativeEditorSDK, captionBlock: number) {
...
export async function createCaptionsFromTranscription(
  cesdk: CreativeEditorSDK,
  ...
  captionBlocks.forEach((block: number, index: number) => {
    try {
    ...
    if (transcriptionResult.language) {
      try {
        cesdk.engine.block.setMetadata(
          block,
          'language',
...
function validateSRTContent(srtContent: string) {
...
export function updateCaptionPositions(
  cesdk: CreativeEditorSDK,
  captionBlocks: number[],
  ...
  captionBlocks.forEach((block: number) => {

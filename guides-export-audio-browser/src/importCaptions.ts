export async function importCaptionsToScene(
  ...
  try {
    ...
    const captionBlocks = await cesdk.engine.block.createCaptionsFromURI(
      captionURI
    );
...
function getCaptionTrack(cesdk) {
  ...
  return children.find(
    (block) => cesdk.engine.block.getType(block) === '//ly.img.ubq/captionTrack'
  );
}

...
export const copyAssets = (engine, fromPageId, toPageId) => {
  ...
  for (
    ...
    // TODO: There seems to be a bug, where engine.block.getBool(fromBlock, 'image/showsPlaceholderOverlay') returns the wrong result
    // So to hotfix this, we just look at the fileUri
    if (
      fromImageFileUri ===
      'https://firebasestorage.googleapis.com/v0/b/cesdk-ab4f3.appspot.com/o/dashboard%2Fuploads%2Fdesign%2FSIFrajHmsMHxTzuFD6Jt%2F51981679-b9ad-4f83-bb23-56970ee16122?alt=media&token=885c9368-1404-4763-8bc9-737e26fa0762'
    ) {
      engine.block.setBool(
        toBlock,
        'image/showsPlaceholderOverlay',
        engine.block.getBool(fromBlock, 'image/showsPlaceholderOverlay')
      );
      engine.block.setBool(
        toBlock,
        'image/showsPlaceholderButton',
        engine.block.getBool(fromBlock, 'image/showsPlaceholderButton')
      );
    } else {

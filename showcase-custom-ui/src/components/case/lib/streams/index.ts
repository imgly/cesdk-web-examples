...
export const filter = <T extends any>(filterFn: (e: T) => boolean) => (
  source: Source<T>
...
export const map = <I, O>(mapFn: (i: I) => O) => (source: Source<I>) => (
  listener: Handler<O>
): UnsubscribeFn => {
  return source((e) => {
    listener(mapFn(e));
 ...
 * @returns
 */
export const mapAsync = <I, O = I>(mapFn: (i: I) => O | Promise<O>) => (
 ...
 */
export const kickOff = <T>(firstValue: T | (() => T)) => (
  source: Source<T>
) => (handler: Handler<T>): UnsubscribeFn => {
 ...
 */
export const memo = <T>(
  equals = (value: T, lastValue: T | undefined) => value === lastValue
) => (source: Source<T>) => {
  let lastValue: T;

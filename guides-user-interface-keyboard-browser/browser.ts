...
class KeyboardExample implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    const allShortcuts = cesdk.shortcuts.list({ keys: '*', scopes: '*' });
    ...
    // `list`, `get`, and `remove` take a `{ keys, scopes }` selector; a shortcut
    // matches when its keys AND scopes both match. Each is a glob (or array of
    // globs); `'*'` matches everything, `[]` matches nothing.

    // Every Mod+<key> shortcut in the canvas scope.
    const modCanvas = cesdk.shortcuts.list({
      keys: 'Mod+*',
